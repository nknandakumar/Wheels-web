  ...
  if (leads.length === 0) {
    return (
      <div className="text-center p-8 border rounded-lg bg-card">
        ...
        <p className="text-muted-foreground">Get started by creating a new lead.</p>
      </div>
    );
  }
        ...
        <TableBody>
          {leads.map((lead) => (
            <TableRow key={lead.id}>
              ...
              <TableCell>{new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(lead.loanAmount)}</TableCell>
              <TableCell>
                <Badge variant={getBadgeVariant(lead.stage)}>{lead.stage}</Badge>
              </TableCell>
              <TableCell className="text-right">
                <Button variant="outline" size="sm" onClick={() => router.push(`/dashboard/my-leads/${lead.id}`)}>
                  View/Edit
                </Button>
              </TableCell>
            </TableRow>
